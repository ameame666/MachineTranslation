...
def get_word_dict():
    ...
    en_idx2word = {}
    en_word2idx = {}
    with open("MachineTranslation/data/word_name_dict/cn_index_dict.csv", 'r', encoding="utf-8") as f:
        reader = csv.reader(f)
        data = list(reader)
        for l in data:
            cn_idx2word[int(l[0])] = l[1]
            cn_word2idx[l[1]] = int(l[0])
    with open("MachineTranslation/data/word_name_dict/en_index_dict.csv", 'r', encoding="utf-8") as f:
        reader = csv.reader(f)
        data = list(reader)
        for l in data:
...
# 保存参考译文到文件中
def bleu_references(read_filename, save_en, save_cn):
    writer_en = open(save_en, 'a+', encoding='utf-8')
    writer_cn = open(save_cn, 'a+', encoding='utf-8')
    with open(read_filename, 'r', encoding="utf-8") as f_read:
        for line in f_read:
            line = line.strip().split('\t')
            sentence_en = "".join([w for w in line[0]])
            sentence_cn = " ".join([w for w in line[1]])
            writer_en.write(sentence_en + '\n')
            writer_cn.write(sentence_cn + '\n')
    writer_en.close()
    writer_cn.close()
    print('写入成功')
